...
ID3D12RootSignature* InitRootSignature()
{
	...
	rootParameters[0].Constants.RegisterSpace = 0; //这是 “寄存器空间”  给复杂引擎用的 会细分很多不同的space  我们简单的填0
	rootParameters[0].Constants.ShaderRegister = 0; //对于shader 那边的 cbuffer 那边的 b0 的 0
	rootParameters[0].Constants.Num32BitValues = 4;//有多少个32位（4字节）的数据？   这个根常量里有 4 个 float（因为 1 个 float = 32 位 = 4字节）



	//CBV就占2个DWORD 
	rootParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;  //给CBV用的了 Constant Buffer（
	rootParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL; //改为all
	rootParameters[1].Descriptor.RegisterSpace = 0; //这是 “寄存器空间”  给复杂引擎用的 会细分很多不同的space  我们简单的填0
	rootParameters[1].Descriptor.ShaderRegister = 1; //这边给1 因为0号已经被上面占了
...
ID3D12Resource* CreateConstantBufferObject(int inDataLen)
{
	...
	gD3D12Device->CreateCommittedResource(
		...
		&d3d12ResourceDesc,
		D3D12_RESOURCE_STATE_GENERIC_READ, //要被shader里读取的
...
void UpdateConstantBuffer(ID3D12Resource* inCB, void* inData, int inDataLen)
{
	...
	D3D12_RANGE d3d12Range = { 0 }; //?  这个结构体的成员分别是Begin和End，表示CPU访问内存的范围。对于常量缓冲区，我们通常不需要CPU读取，所以可以设置为{0}，表示没有有效的读取范围。

	unsigned char* pBuffer = nullptr; //这个指针用来接收 Map 函数返回的内存地址，CPU 将通过这个地址访问常量缓冲区的数据。



	/////Map -> memcpy -> unmap 流程
	///Map 是为了打破 CPU 和 GPU 的次元壁。

	inCB->Map(0, &d3d12Range, (void**)&pBuffer); //调用 Map 函数将常量缓冲区资源映射到 CPU 可访问的内存地址。第一个参数 0 表示映射第一个子资源，二个参数是读取范围，第三个参数是接收映射地址的指针。

	memcpy(pBuffer, inData, inDataLen); //使用 memcpy 函数将新的数据从 inData 复制到映射的内存地址 pBuffer。inDataLen 指定了复制的数据大小。

	inCB->Unmap(0, nullptr); //调用 Unmap 函数解除资源的映射。第一个参数 0 表示解除映射第一个子资源，第二个参数 nullptr 表示没有需要写回的数据范围（因为我们已经通过 memcpy 更新了数据）。
...
///用来创建VBO 即 vertex buffer object的函数   也可以用来创建IBO 这是个通用的创建buffer资源的函数
...
ID3D12Resource* CreateTexture2D(ID3D12GraphicsCommandList* inCommandList)
{
	...
	d3d12TempResourceDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER; //是Texture 
